...
def run_multihead_self_attention(
    ...
    """
    mutil_head_self_attention = MultiheadSelfAttention(d_model, num_heads)
    mutil_head_self_attention.load_state_dict({
        "q_proj.weight": q_proj_weight,
        "k_proj.weight": k_proj_weight,
        "v_proj.weight": v_proj_weight,
        "o_proj.weight": o_proj_weight
    })
    return mutil_head_self_attention(in_features)
...
def run_transformer_block(
    ...
    re_weights = {
        "attn.o_proj.weight" if k == "attn.output_proj.weight" else k: v for k, v in weights.items()
    }
    state_dict = {
        **re_weights,
        "attn.rope.cos": block.attn.rope.cos,
        "attn.rope.sin": block.attn.rope.sin,
    }
...
def run_transformer_lm(
    ...
    re_weights = {                                                             
         k.replace("output_proj.weight", "o_proj.weight"): v                    
         for k, v in weights.items()                                            
     }
    state_dict = dict(re_weights)
    for i in range(num_layers):
        state_dict[f"layers.{i}.attn.rope.cos"] = model.layers[i].attn.rope.cos
        state_dict[f"layers.{i}.attn.rope.sin"] = model.layers[i].attn.rope.sin
